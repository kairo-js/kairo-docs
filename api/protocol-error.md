...
| プロパティ | 型 | 説明 |
|---|---|---|
| `source` | `"local_parse" \| "remote"` | エラー発生箇所。`"local_parse"` は自アドオン側のパース失敗、`"remote"` は送信元アドオンから報告されたエラー |
| `protocolStage` | `ProtocolStage \| undefined` | エラーが発生したプロトコルステージ |
| `correlationId` | `string \| undefined` | 対象 `request()` の correlationId |
